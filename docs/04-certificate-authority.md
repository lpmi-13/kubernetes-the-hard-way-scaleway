...
We need to do a bit of digging to get the controller private NIC IPs (Scaleway provides a default IP that doesn't go away even after you attach the instance to a private network, so it's a bit confusing, and we don't want the private IP that comes back when we query the actual instance)
